...
module.exports.updateUser = async (data, userId, transaction) => {
  const [updatedCount, [updatedUser]] = await dbUser.update(data,
...
module.exports.findUser = async (predicate, transaction) => {
  const result = await dbUser.findOne({ where: predicate, transaction });
  if (!result) {
...
module.exports.userCreation = async (data) => {
  const newUser = await dbUser.create(data);
  if (!newUser) {
