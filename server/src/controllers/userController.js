...
module.exports.changeMark = async (req, res, next) => {
  ...
  try {
    transaction = await dbsequelize.transaction(
      { isolationLevel: dbSequelize.Transaction.ISOLATION_LEVELS.READ_UNCOMMITTED });
...
module.exports.cashout = async (req, res, next) => {
  ...
  try {
    ...
    const updatedUser = await userQueries.updateUser(
      { balance: dbsequelize.literal('balance - ' + req.body.sum) },
      req.tokenData.userId, transaction);
    await bankQueries.updateBankBalance({
      balance: dbsequelize.literal(`CASE 
                WHEN "cardNumber"='${ req.body.number.replace(/ /g,
